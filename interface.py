...
class DeadlockToolkitGUI:
    def __init__(self, root):
        ...
        self.btn_visualize = tk.Button(root, text="Visualize Resource Allocation Graph", command=self.visualize_rag, bg="purple", fg="white")
    ...
    def run_deadlock_prevention(self):
        try:
            ...
            max_claim = self.parse_allocations()  # For now, using same input. Should add a separate input.
            ...
            if safe:
                messagebox.showinfo("Deadlock Prevention", f"System is in a safe state.\nSafe sequence: {sequence}")
            else:
    ...
    def visualize_rag(self):
        try:
            ...
            for i, row in enumerate(allocations):
                for j, alloc in enumerate(row):
                    if alloc > 0:
                        G.add_edge(processes[i].strip(), resources[j].strip())  # Process -> Resource
